...
for runs=1:20
    ...
    if kva ~=1
        ...
        %% VALIDATION
        [phiva] = phi_fun(c,xvanorm,N, size(xva,1),size(xva,2),SIGMA);
                
        itytruevanorm=phiva*w;
        [itytrueva] = unnormalization(a, b, minvarytr, maxvarytr, itytruevanorm, size(yva,1));
             
        RMSEva = sqrt(mean((yva-itytrueva).^2));   
                
        %% TESTING
        [phite] = phi_fun(c,xtenorm,N, size(xte,1),size(xte,2),SIGMA);
                
        itytruetenorm=phite*w;
        [itytruete] = unnormalization(a, b, minvarytr, maxvarytr, itytruetenorm, size(yte,1));
